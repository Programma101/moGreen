...
def invioFotoSuWeb():
    ...
    ftp = ftplib.FTP('INSERIREINDIRIZZOSPAZIOWEB','INSERIREQUIDIRECTORYSPAZIOEWB','INSERIREQUIPASSWORDSPAZIOWEB') # Si connette
    ...
    print("ultima.jpg inviata a INSERIEQUISPAZIOEDIRECTORYWEB/ultima.jpg")
...
def copiaUltimaInFotoDiPiante():
    ...
    path="/media/pi/INDIRIZZOCHIAVINAUSB/FotoDiPiante"
    ...
    if os.listdir(path)==[]:
        ...
        path2="/media/pi/INDIRIZZOCHIAVINAUSB/FotoDiPiante"
        print(nuovoNome)
...
def mediaDatiGiornalieri():
    ...
    if ultima48ore<25:
        ...
        #questa funzione potrebbe scattare a mezzanotte e fare la media dei dati del giorno precedente
        #come al solito volevo una scorciatoia
        #e fare il calcolo media giornaliera su excell invece che con Python ma
        #non funziona poi la copia del dato in altri fogli
        #perchÉ openpyxl fa schifo
        dataOraG=ws1.cell(row=ultima48ore, column=1).value#questa va bene, in effetti no, nom mi piace devo togliere l'ora
        #dataOraG=dataOraG.strftime('%d/%m/%Y')#dovrei avere tolto l'ora, non è così in quanto non è una data ma una stringa
        #devo togliere le ultime lettere ad una stringa
        data=dataOraG[0:10]
        ...
        ws5.append([data,TAmbG,URG,LuceG,BS10G,TS10G,BS30G,TS30G])
...
def aggiornaDatiGiornalieriSuGSheet():       
    ...
    print("Gli ultimi dati giornalieri sono ",data, TAmbG,URG,LuceG,BS10G,TS10G,BS30G,TS30G," e li sto scrivendo in Gsheet")
    wks4.append_row([data,TAmbG,URG,LuceG,BS10G,TS10G,BS30G,TS30G])#aggiorno il foglio dati giornalieri su gsheet
...
def scrivoDatiGiornalieriInAltriFogli(foglio, nMaxRighe):
    ...
    if  urdg<2:
        ...
        print ("i dati giornalieri sono: ", dataOraG,TAmbG,URG,LuceG,BS10G,TS10G,BS30G,TS30G, " e sono stati inseriti alla riga ",foglio.max_row+1)
        #time.sleep(1)
        ...
        foglio.move_range("A3:H"+str(nMaxRighe),rows=-1)
...
def daStoreAGsheet(wsstore,wsgsheet,colonnastore):#devo sostituire i valori alle variabili della funzione    
    ...
    for n in range(2,rigastore+1):#faccio un ciclo che ripete le azioni per ogni riga   
        ...
        for m in range (2,colonnastore+1): #il ciclo per ogni colonna    
            i=7 #Per metterlo in ordine giusto nella lista parto dall'indice più alto (non ho capito bene perchè)
            #print(m)
...
def creaVideo():
    ...
    dst='/media/pi/45D5-C1E63/'+newDir
    #time.sleep(1)
    ...
    os.rename('/media/pi/INDIRIZZOCHIAVINAUSB/FotoDiPiante', dst)
    #time.sleep(1)
    ...
    os.mkdir('/media/pi/INDIRIZZOCHIAVINAUSB/FotoDiPiante') #si possono specificare anche i permessi ma al momento
...
def mandaVideoSuWeb():
    ...
    session=ftplib.FTP('INDIRIZZOSPAZIOWEB', 'DIRECTORYWEB','PASSWORDSPAZIOWEB')
    ...
    body="INDIRIZZOSPAZIOEDIRECTORYWEB/"+nomefile1
    time.sleep(1)
...
def sendAlarmBagnaturaSuolo():   
    ...
    if r<24:
        ...
        smtpObj= smtplib.SMTP('smtp.GESTOREMAIL', 587)
        ...
        smtpObj.login('INDIRIZZOMAIL', 'PASSWORDMAIL')
        ...
        if (eMail1!="None") and (statoIdrico!="acquaOk"):
            ...
            smtpObj.starttls()
            smtpObj.login('INDIRIZZOMAIL', 'PASSWORDMAIL')
            smtpObj.sendmail('INDIRIZZOMAIL', eMail1, "subject: "+oggetto)
        else:
        ...
        if (eMail2!="None") and (statoIdrico!="acquaOk"):
            ...
            smtpObj.starttls()
            smtpObj.login('INDIRIZZOMAIL', 'PASSWORDMAIL')
            smtpObj.sendmail('INDIRIZZOMAIL', eMail2, "subject: "+oggetto)
        else:
            print("non ci sono le condizioni per l'invio del messaggio")       
        if (eMail3!="None") and (statoIdrico!="acquaOk"):
            ...
            smtpObj.starttls()
            smtpObj.login('INDIRIZZOMAIL', 'PASSWORDMAIL')
            smtpObj.sendmail('INDIRIZZOMAIL', eMail3, "subject: "+oggetto)
        else:
            print("non ci sono le condizioni per l'invio del messaggio")
        del smtplib    
...
def restoreGsheet():
    ...
    server = IMAPClient('IMAPCLIENT', use_uid=True)
    server.login('INDIRIZZOMAIL', 'PASSWORDMAIL')#credenziali di plantOut
...
def rimandaVideoSuWeb():
    from imapclient import IMAPClient
    server = IMAPClient('imap.GESTOREMAIL', use_uid=True)
    server.login('INDIRIZZOMAIL', 'PASSWORDMAIL')#credenziali di plantOut
    ...
    if nmsg!=0:
        ...
        import imapclient
        import imaplib
        imapObj=imapclient.IMAPClient("imap.GESTOREMAIL", 993)
        imapObj.login('INDIRIZZOMAIL', 'PASSWORDMAIL')
        imapObj.select_folder('INBOX', readonly=False)
        imapObj.delete_messages(UIDs)
        imapObj.expunge()
        print("messaggio cancellato")
...
def copiaERinominaStore():
    ...
    dst="/home/pi/plant+out/nuoveProve/store.xlsx"
    shutil.copy2(src, dst)
    print("ho copiato store nella directory nuoveProve")
    vecchioNome = "/home/pi/plant+out/nuoveProve/store.xlsx"
    nuovoNome = "/home/pi/plant+out/nuoveProve/"+newFile
    path1="/home/pi/plant+out/nuoveProve/"+newFile
    path2="/media/pi/INDIRIZZOCHIAVINAUSB/copiaStore"
    os.rename(vecchioNome,nuovoNome)
    print("Ho rinominato store nella directory nuoveProve "+newFile)
    shutil.move(path1,path2)
    print("Ho spostato "+newFile+ " in "+path2)
    del datetime
    del os
    del shutil
...
try:
    ...
    print (ora)
    filename=ora+"errore_recuperoRilieviDaStore.txt"
    print(filename)
...
if check_internet_connection():
    ...
    credentials = ServiceAccountCredentials.from_json_keyfile_name('/home/pi/plant+out/NOMEDELKEYFILE.json', scope)
    print (credentials)
    ...
    print("Nessuna connessione a Internet per la copia di sheet1 su gSheet.")
    import datetime
    ora=datetime.datetime.now()
    ora=ora.strftime("%H:%M:%S")
    print (ora)
    filename=ora+"erroreCopiaDatiSheet1SuGSheetPerMancataConnessione.txt"
    print(filename)
    #e lo inserisce in una specifica cartella "errori"
    errorFile=open ('/home/pi/plant+out/errori/'+str(filename) ,'w')
    errorFile.close()
...
try:
    ...
    ora=ora.strftime("%H:%M:%S")
    print (ora)
    filename=ora+"erroreFotoCondizioneLux.txt"
    print(filename)
    #e lo inserisce in una specifica cartella "errori"
    errorFile=open ('/home/pi/plant+out/errori/'+str(filename) ,'w')
    errorFile.write(traceback.format_exc())
...
if check_internet_connection():
    # Solo se c'è la connessione, verifica la seconda condizione
    print(minu)
    if minu=="00"or minu=="30":
        ...
        try:
            ...
            ora=ora.strftime("%H:%M:%S")
            print (ora)
            filename=ora+"erroreEsecuzioneInvioFotoSuWeb.txt"
            print(filename)
            #e lo inserisce in una specifica cartella "errori"
            errorFile=open ('/home/pi/plant+out/errori/'+str(filename) ,'w')
            errorFile.write(traceback.format_exc())
    ...
    print("Nessuna connessione a Internet per invioFotoSuWeb. passo a considerare lo script succcessivo.")
    import datetime
    ora=datetime.datetime.now()
    ora=ora.strftime("%H:%M:%S")
    print (ora)
    filename=ora+"erroreEsecuzioneInviofotoSuWebPerMancataConnessione.txt"
    print(filename)
    #e lo inserisce in una specifica cartella "errori"
    errorFile=open ('/home/pi/plant+out/errori/'+str(filename) ,'w')
    errorFile.close()
...
try:
    ...
    ora=ora.strftime("%H:%M:%S")
    print (ora)
    filename=ora+"erroreCopiaUltimaInFotoDiPiante.txt"
    print(filename)
    #e lo inserisce in una specifica cartella "errori"
    errorFile=open ('/home/pi/plant+out/errori/'+str(filename) ,'w')
    errorFile.write(traceback.format_exc())
...
try:
    ...
    ora=ora.strftime("%H:%M:%S")
    print (ora)
    filename=ora+"erroreEsecuzioneScrivi48oreStore.txt"
    print(filename)
    #e lo inserisce in una specifica cartella "errori"
    errorFile=open ('/home/pi/plant+out/errori/'+str(filename) ,'w')
    errorFile.write(traceback.format_exc())
...
if check_internet_connection():
    ...
    if minu=="00":
        ...
        try:
            ...
            ora=ora.strftime("%H:%M:%S")
            print (ora)
            filename=ora+"erroreEsecuzioneCopioFoglio48oreSugSheet.txt"
            print(filename)
             #e lo inserisce in una specifica cartella "errori"
            errorFile=open ('/home/pi/plant+out/errori/'+str(filename) ,'w')
            errorFile.write(traceback.format_exc())
    ...
    print("Nessuna connessione a Internet. passo a considerare per la funzione copia 48 ore su gSheet.")
    import datetime
    ora=datetime.datetime.now()
    ora=ora.strftime("%H:%M:%S")
    print (ora)
    filename=ora+"erroreEsecuzioneCopioFoglio48oreSugSheetPerMancataConnessione.txt"
    print(filename)
    #e lo inserisce in una specifica cartella "errori"
    errorFile=open ('/home/pi/plant+out/errori/'+str(filename) ,'w')
    errorFile.close()
...
if check_internet_connection():
    ...
    if minu=="00":
        ...
        try:
            ...
            ora=ora.strftime("%H:%M:%S")
            print (ora)
            filename=ora+"erroreEsecuzioneCopioFoglioDatiOrariSugSheet.txt"
            print(filename)
            #e lo inserisce in una specifica cartella "errori"
            errorFile=open ('/home/pi/plant+out/errori/'+str(filename) ,'w')
            errorFile.write(traceback.format_exc())
    ...
    print("Nessuna connessione a Internet per la copia dati orari su gSheet. passo a considerare lo script succcessivo.")
    import datetime
    ora=datetime.datetime.now()
    ora=ora.strftime("%H:%M:%S")
    print (ora)
    filename=ora+"erroreEsecuzioneCopioFoglioDatiOrariSugSheetPerMancataConnessione.txt"
    print(filename)
    #e lo inserisce in una specifica cartella "errori"
    errorFile=open ('/home/pi/plant+out/errori/'+str(filename) ,'w')
    errorFile.close()
...
if check_internet_connection():
    ...
    if minu=="45":
        ...
        try:
            ...
            ora=ora.strftime("%H:%M:%S")
            print (ora)
            filename=ora+"erroreEsecuzioneRestoreGsheet.txt"
            print(filename)
            #e lo inserisce in una specifica cartella "errori"
            errorFile=open ('/home/pi/plant+out/errori/'+str(filename) ,'w')
            errorFile.write(traceback.format_exc())
    ...
    print("Nessuna connessione a Internet nel non posso lanciare restoreGsheet. Passo a considerare lo script succcessivo.")
    import datetime
    ora=datetime.datetime.now()
    ora=ora.strftime("%H:%M:%S")
    print (ora)
    filename=ora+"erroreEsecuzioneRestoreGsheetPerMancataConnessione.txt"
    print(filename)
    #e lo inserisce in una specifica cartella "errori"
    errorFile=open ('/home/pi/plant+out/errori/'+str(filename) ,'w')
    errorFile.close()
...
if check_internet_connection():
    ...
    if minu=="45":
        ...
        try:
            ...
            ora=ora.strftime("%H:%M:%S")
            print (ora)
            filename=ora+"erroreEsecuzioneRimandaVideoSuWeb.txt"
            print(filename)
            #e lo inserisce in una specifica cartella "errori"
            errorFile=open ('/home/pi/plant+out/errori/'+str(filename) ,'w')
            errorFile.write(traceback.format_exc())
    ...
    print("Nessuna connessione a Internet non posso lanciare rimandaVideoSuWeb. Passo a considerare lo script succcessivo.")
    import datetime
    ora=datetime.datetime.now()
    ora=ora.strftime("%H:%M:%S")
    print (ora)
    filename=ora+"erroreEsecuzioneRimandoVideoSuWebPerMancataConnessione.txt"
    print(filename)
    #e lo inserisce in una specifica cartella "errori"
    errorFile=open ('/home/pi/plant+out/errori/'+str(filename) ,'w')
    errorFile.close()
...
try: 
    ...
    ora=ora.strftime("%H:%M:%S")
    print (ora)
    filename=ora+"erroreEsecuzioneFunzioneMediaDatiGiornalieriSuStore.txt"
    print(filename)
    #e lo inserisce in una specifica cartella "errori"
    errorFile=open ('/home/pi/plant+out/errori/'+str(filename) ,'w')
    errorFile.write(traceback.format_exc())
...
try:
    ...
    ora=ora.strftime("%H:%M:%S")
    print (ora)
    filename=ora+"erroreEsecuzioneFunzioneCopioDatiGiornalieriInultime2settimane.txt"
    print(filename)
    #e lo inserisce in una specifica cartella "errori"
    errorFile=open ('/home/pi/plant+out/errori/'+str(filename) ,'w')
    errorFile.write(traceback.format_exc())
...
try:
    ...
    ora=ora.strftime("%H:%M:%S")
    print (ora)
    filename=ora+"erroreEsecuzioneFunzioneCopioDatiGiornalieriInUltimaSettimana.txt"
    print(filename)
    #e lo inserisce in una specifica cartella "errori"
    errorFile=open ('/home/pi/plant+out/errori/'+str(filename) ,'w')
    errorFile.write(traceback.format_exc())
...
try:
    ...
    ora=ora.strftime("%H:%M:%S")
    print (ora)
    filename=ora+"erroreEsecuzioneFunzioneCopioDatiGiornalieriInUltimoMese.txt"
    print(filename)
    #e lo inserisce in una specifica cartella "errori"
    errorFile=open ('/home/pi/plant+out/errori/'+str(filename) ,'w')
    errorFile.write(traceback.format_exc())
...
if check_internet_connection():
    ...
    if ora=="00"and minu=="15":
        try:
            ...
            ora=ora.strftime("%H:%M:%S")
            print (ora)
            filename=ora+"erroreEsecuzioneAggiornaGiornalieriSuGsheet.txt"
            print(filename)
            #e lo inserisce in una specifica cartella "errori"
            errorFile=open ('/home/pi/plant+out/errori/'+str(filename) ,'w')
            errorFile.write(traceback.format_exc())
    ...
    print("Nessuna connessione a Internet per aggiornaGiornalieriSuGsheet. passo a considerare lo script succcessivo.")
    import datetime
    ora=datetime.datetime.now()
    ora=ora.strftime("%H:%M:%S")
    print (ora)
    filename=ora+"erroreEsecuzioneAggiornaGiornalieriSuGsheetPerMancataConnessione.txt"
    print(filename)
    #e lo inserisce in una specifica cartella "errori"
    errorFile=open ('/home/pi/plant+out/errori/'+str(filename) ,'w')
    errorFile.close()
...
if check_internet_connection():
    print("15 connessione attiva per copia ultima settimana su gSheet")   
    print(ora)
    print(minu)
    if ora=="00"and minu=="15":
        try:
            ...
            ora=ora.strftime("%H:%M:%S")
            print (ora)
            filename=ora+"erroreCopiaUltimaSettimanaSuGsheet.txt"
            print(filename)
            #e lo inserisce in una specifica cartella "errori"
            errorFile=open ('/home/pi/plant+out/errori/'+str(filename) ,'w')
            errorFile.write(traceback.format_exc())
    ...
    ora=ora.strftime("%H:%M:%S")
    print (ora)
    filename=ora+"erroreEsecuzioneCopiaUltimaSettimanaSuGsheetPerMancataConnessione.txt"
    print(filename)
    #e lo inserisce in una specifica cartella "errori"
    errorFile=open ('/home/pi/plant+out/errori/'+str(filename) ,'w')
    errorFile.close()
...
if check_internet_connection():
    print("15 connessione attiva per copia ultime 2 settimane su gSheet")   
    print(ora)
    print(minu)
    if ora=="00"and minu=="15":
        try:
            ...
            ora=ora.strftime("%H:%M:%S")
            print (ora)
            filename=ora+"erroreCopiaUltime2settimaneSuGsheet.txt"
            print(filename)
            #e lo inserisce in una specifica cartella "errori"
            errorFile=open ('/home/pi/plant+out/errori/'+str(filename) ,'w')
            errorFile.write(traceback.format_exc())
    ...
    print("Nessuna connessione a Internet per copia ultime 2 settimane SuGsheet. passo a considerare lo script succcessivo.")
    import datetime
    ora=datetime.datetime.now()
    ora=ora.strftime("%H:%M:%S")
    print (ora)
    filename=ora+"erroreEsecuzioneCopiaUltime2settimanePerMancataConnessione.txt"
    print(filename)
    #e lo inserisce in una specifica cartella "errori"
    errorFile=open ('/home/pi/plant+out/errori/'+str(filename) ,'w')
    errorFile.close()
...
if check_internet_connection():
    print("15 connessione attiva per copia ultimo mese su gSheet")   
    print(ora)
    print(minu)
    if ora=="00"and minu=="15":
        try:
            ...
            ora=ora.strftime("%H:%M:%S")
            print (ora)
            filename=ora+"erroreCopiaUltimoMeseSuGsheet.txt"
            print(filename)
            #e lo inserisce in una specifica cartella "errori"
            errorFile=open ('/home/pi/plant+out/errori/'+str(filename) ,'w')
            errorFile.write(traceback.format_exc())
    ...
    print("Nessuna connessione a Internet per copia ultimo mese SuGsheet. passo a considerare lo script succcessivo.")
    import datetime
    ora=datetime.datetime.now()
    ora=ora.strftime("%H:%M:%S")
    print (ora)
    filename=ora+"erroreEsecuzioneCopiaUltimoMesePerMancataConnessione.txt"
    print(filename)
    #e lo inserisce in una specifica cartella "errori"
    errorFile=open ('/home/pi/plant+out/errori/'+str(filename) ,'w')
    errorFile.close()
...
try:
    ...
    ora=ora.strftime("%H:%M:%S")
    print (ora)
    filename=ora+"erroreEsecuzioneFunzioneCoiaERinominaStore.txt"
    print(filename)
    #e lo inserisce in una specifica cartella "errori"
    errorFile=open ('/home/pi/plant+out/errori/'+str(filename) ,'w')
    errorFile.write(traceback.format_exc())
...
if check_internet_connection():
    print("19 connessione attiva per invio mail di allarme bagnatura suolo")   
    print(ora)
    print(minu)
    if ora=="07"and minu=="15":
        try:
            ...
            ora=ora.strftime("%H:%M:%S")
            print (ora)
            filename=ora+"erroreSendAlarmBagnaturaSuolo.txt"
            print(filename)
            #e lo inserisce in una specifica cartella "errori"
            errorFile=open ('/home/pi/plant+out/errori/'+str(filename) ,'w')
            errorFile.write(traceback.format_exc())
    ...
    print("Nessuna connessione a Internet per sendAlarmBagnaturaSuolo. passo a considerare lo script succcessivo.")
    import datetime
    ora=datetime.datetime.now()
    ora=ora.strftime("%H:%M:%S")
    print (ora)
    filename=ora+"erroreEsecuzioneSendAlarm BagnaturaSuoloPerMancataConnessione.txt"
    print(filename)
    #e lo inserisce in una specifica cartella "errori"
    errorFile=open ('/home/pi/plant+out/errori/'+str(filename) ,'w')
    errorFile.close()
...
try:
    ...
    ora=ora.strftime("%H:%M:%S")
    print (ora)
    filename=ora+"erroreEsecuzioneFunzioneCreaVideo.txt"
    print(filename)
    #e lo inserisce in una specifica cartella "errori"
    errorFile=open ('/home/pi/plant+out/errori/'+str(filename) ,'w')
    errorFile.write(traceback.format_exc())
...
if check_internet_connection():
    ...
    if (ora=="04"and minu=="15" and giorno=="Thursday") or (ora=="04"and minu=="15" and giorno=="Sunday"):
        ...
        try:
            ...
            ora=ora.strftime("%H:%M:%S")
            print (ora)
            filename=ora+"erroreEsecuzioneMandaVideoSuWeb.txt"
            print(filename)
            #e lo inserisce in una specifica cartella "errori"
            errorFile=open ('/home/pi/plant+out/errori/'+str(filename) ,'w')
            errorFile.write(traceback.format_exc())
    ...
    print("Nessuna connessione a Internet per mandare video su web ma posso mandare mail con oggetto video quando la connessione sarà attiva.")
    import datetime
    ora=datetime.datetime.now()
    ora=ora.strftime("%H:%M:%S")
    print (ora)
    filename=ora+"erroreEsecuzioneMandaVideoSuWebPerMancataConnessione.txt"
    print(filename)
    #e lo inserisce in una specifica cartella "errori"
    errorFile=open ('/home/pi/plant+out/errori/'+str(filename) ,'w')
    errorFile.close()
